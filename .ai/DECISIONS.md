...
## 2026-08-30 — 시장 데이터는 DB에 쌓고, 외부(야후)는 하루 한 번만 부른다
Context: 종목 상세·관심종목을 붙이며 yfinance를 화면에서 직접 호출했다. 관심종목 목록이 종목 수만큼 외부 호출을 했고, 야후가 죽으면 화면이 비었다. 프로세스 메모리 캐시는 워커별로 따로였고 재시작하면 날아갔다.
Decision: **읽기와 채우기를 가른다.** 화면(`market.candles`/`profile`)은 `symbol_candles`·`symbol_meta`·`symbol_dividends`만 읽는다. 채우기는 크론 하루 1회(07:10 `cli.py market-refresh`)가 하고, 실패는 `failed_at`을 남겨 6시간 뒤 재시도한다. 처음 담은 종목만 예외로 그 자리에서 한 번 받는다. 일봉만 저장하고 주봉·월봉은 읽을 때 만든다(이력을 한 벌만 관리).
Reason: 외부 의존을 화면에서 걷어내면 야후가 죽어도, 관심종목이 서른 개라도 즉시 뜬다(93ms, 외부 호출 0). 시세는 지나면 되받아오는 비용이 크므로 쌓아 두는 편이 싸다.
Consequences: DB가 커진다(일봉 21.8만행 38MB, 종목당 ~7천행). 데이터가 하루 묵을 수 있다(지연시세라 무방). 야후에 없는 종목은 FDR로 폴백하되 배당·기업정보는 못 채운다. 평가용 시세(FDR, `prices` 테이블)와는 계속 분리 — 여기가 죽어도 순자산은 안 흔들린다.

## 2026-08-30 — 차트는 TradingView Lightweight Charts 하나로, 벤더링해서 쓴다
Context: 캔버스·SVG로 직접 그린 차트 다섯 개가 축 눈금 없이 최고·최저 두 값만 찍고 있었다. 크로스헤어·거래량·시간축을 손으로 만들 일이 아니다.
Decision: 시계열 차트 전부를 Lightweight Charts(Apache-2.0)로 통일하고, 파일을 `app/static/vendor/`에 커밋해 CDN 없이 쓴다. 공통 설정은 `lcCreate()`/`lcTheme()` 한 곳. 색은 우리 6색 팔레트(오르면 빨강·내리면 파랑).
Reason: 집 서버라 바깥이 끊겨도 화면은 떠야 하고, CDN 버전이 말없이 바뀌면 안 된다. 한 화면에 축 모양이 두 가지인 게 손그림보다 나쁘다.
Consequences: 182KB가 저장소에 들어온다. 도넛·비율 막대는 이 라이브러리가 시계열 전용이라 대상이 아니다 — 원그래프까지 통일하려면 다른 라이브러리를 더 들여야 하는데, 그건 엔진이 둘이 되는 값을 치를 만하지 않다고 봤다.
